...
async function login(role){const accounts={superadmin:["superadmin@school-ing.gn","SuperAdmin123!"],admin:["admin@school-ing.gn","ChangeMe123!"],teacher:["teacher@school-ing.gn","Teacher123!"],student:["student@school-ing.gn","Student123!"],parent:["parent@school-ing.gn","Parent123!"]},[email,password]=accounts[role],platform=role==="superadmin",module=platform?modules.platformLogin:modules.schoolLogin,path=platform?"/api/platform/auth/login":"/api/auth/login",body=platform?{email,password}:{tenantId:"demo-school",email,password};const response=await module.routeModule.userland.POST(new Request(`${base}${path}`,{method:"POST",headers:{"content-type":"application/json",origin:base},body:JSON.stringify(body)}));check(response.status===200,`${role} login`);return (response.headers.get("set-cookie")||"").split(";")[0]}
...
const studentBoard=await call(modules.board,"/api/gamification/leaderboard",cookies.student);check(studentBoard.response.status===200,"student leaderboard loads");check(studentBoard.payload?.data?.overall===null,"private global leaderboard hidden from student");check(studentBoard.payload?.data?.classes?.length===1&&studentBoard.payload.data.classes[0]==="10e-A","student leaderboard limited to own class");
const parentBoard=await call(modules.board,"/api/gamification/leaderboard",cookies.parent);check(parentBoard.payload?.data?.classes?.length===1&&parentBoard.payload.data.classes[0]==="10e-A","parent leaderboard limited to child class");
const teacherBoard=await call(modules.board,"/api/gamification/leaderboard",cookies.teacher);check(teacherBoard.payload?.data?.classes?.every(value=>["7e-A","10e-A"].includes(value)),"teacher leaderboard limited to assigned classes");
...
const teacherStudents=await call(modules.students,"/api/students",cookies.teacher);check(teacherStudents.payload?.data?.every(student=>["7e-A","10e-A"].includes(student.className)),"teacher directory limited to assigned classes");
