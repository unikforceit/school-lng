...
const card=readFileSync("src/app/(dashboard)/id-card/page.tsx","utf8"),cardUi=readFileSync("src/components/IdCard.tsx","utf8");check(card.includes("schoolScope(session)")&&card.includes('source:"student"'),"Smart Card role scope enforced server-side");check(cardUi.includes("Blood type")&&cardUi.includes("Print / Save PDF")&&cardUi.includes("qrCode"),"Smart Card includes printable identity and QR fields");
restoreSettings();
