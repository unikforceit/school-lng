...
export async function POST(request:Request,context:{params:Promise<{type:string}>}){
 ...
 const insert=db.prepare("INSERT INTO resources (tenant_id,resource_type,title,payload) VALUES (?,?,?,?)");db.transaction(()=>{for(const row of prepared)insert.run(session.tenantId,type.data,row.title,JSON.stringify(row.payload))})();return Response.json({data:{imported:prepared.length}},{status:201});
}
