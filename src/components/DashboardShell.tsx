...
export default function DashboardShell({ children, userId, role, tenantId, apiVisible = false }: Props) {
  ...
  return <div className="flex min-h-screen bg-[#f8f7f3] text-[#102039]">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 overflow-y-auto border-r border-[#efede7] bg-white px-4 py-3 lg:block">
      <Brand role={role} />
      <Menu role={role} apiVisible={apiVisible} />
    </aside>

    {menuOpen && <div className="fixed inset-0 z-[70] lg:hidden" role="presentation">
      <button type="button" aria-label="Close navigation" className="absolute inset-0 h-full w-full bg-slate-950/45 backdrop-blur-[2px]" onClick={() => setMenuOpen(false)} />
      <aside ref={drawer} id="mobile-navigation" role="dialog" aria-modal="true" aria-label="Main navigation" className="relative h-full w-[min(21rem,88vw)] overflow-y-auto border-r border-slate-200 bg-white px-4 py-3 shadow-2xl">
        <div className="flex items-center justify-between">
          <Brand role={role} onNavigate={() => setMenuOpen(false)} />
          <button ref={closeButton} type="button" onClick={() => setMenuOpen(false)} aria-label="Close menu" className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 text-2xl text-slate-600 hover:bg-slate-50">×</button>
        </div>
