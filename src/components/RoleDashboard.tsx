...
async function loadJson(path: string) {
  ...
  if (!r.ok) throw new Error(p?.error || `Unable to load ${path}`);
  return p.data;
}
...
export default function RoleDashboard({
    ...
    void (async () => {
      try {
        const [
          ...
          loadJson("/api/resources/lessons"),
          loadJson("/api/resources/events"),
          loadJson("/api/resources/announcements"),
          loadJson("/api/resources/assignments"),
          loadJson("/api/resources/attendance"),
          loadJson("/api/resources/results"),
        ]);
        if (active)
          setData({
            dashboard,
  ...
  const stats = useMemo(() => {
    ...
    const counts = new Map(
      data.dashboard.resourceCounts.map((item) => [item.type, item.count]),
    );
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-xl bg-gradient-to-r from-[#d99600] to-[#ffb81e] p-6 text-white">
        <p className="text-sm text-white/90">Welcome back</p>
        <h1 className="mt-1 text-2xl font-bold">{name}</h1>
        <p className="mt-2 text-sm text-white/90">
          Live, role-aware information from your school database.
        </p>
      </section>
      <section
      ...
      <div className="grid gap-5 xl:grid-cols-[1.4fr_1fr]">
        <section className="rounded-xl bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold">Schedule and lessons</h2>
              <p className="text-xs text-slate-500">Current database records</p>
            </div>
            <Link
              href="/list/lessons"
              className="text-sm font-semibold text-[#9a6900]"
            >
              View all
            </Link>
          </div>
          <div className="mt-4 divide-y">
              ...
              >
                <span>
                  ...
                  <span className="text-xs text-slate-500">
                    Event · {String(item.payload.class || "All")}
                  </span>
                </span>
            ...
            {data.lessons.slice(0, 6).map((item) => (
              <Link
                href="/list/lessons"
                key={`lesson-${item.id}`}
                className="flex justify-between gap-4 py-3"
              >
                <span>
                  <strong className="block text-sm">
                    {String(item.payload.subject || item.title)}
                  </strong>
                  <span className="text-xs text-slate-500">
                    Lesson · {String(item.payload.teacher || "")}
                  </span>
                </span>
                <span className="text-xs text-slate-500">
                  {String(item.payload.class || "")}
                </span>
              </Link>
            ))}
            {!data.events.length && !data.lessons.length && (
          ...
          <div className="mt-4 space-y-3">
            {data.announcements.slice(0, 6).map((item) => (
              <Link
                href="/list/announcements"
                key={item.id}
                className="block rounded-xl bg-[#fffaf0] p-4"
              >
                <strong className="text-sm">{item.title}</strong>
                <span className="mt-1 block text-xs text-slate-500">
                  {String(item.payload.class || "All")} ·{" "}
                  {String(item.payload.date || "")}
                </span>
              </Link>
            ))}
